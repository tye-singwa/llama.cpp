        ...
        case LLM_ARCH_MODERNBERT:
            {
                ...
                if (hparams.n_layer == 22 && hparams.n_embd == 768) {
                    type = LLM_TYPE_149M; // modernbert-embed-base
                }
            } break;
        case LLM_ARCH_BLOOM:
            {
            ...
            case LLM_ARCH_MODERNBERT:
                {
                    ...
                    if (arch == LLM_ARCH_BERT || arch == LLM_ARCH_NOMIC_BERT) {
                    ...
                    } else if (arch == LLM_ARCH_MODERNBERT) {
                        output_norm   = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd}, 0);
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (arch == LLM_ARCH_BERT) {
                            ...
                            layer.ffn_up_b   = create_tensor(tn(LLM_TENSOR_FFN_UP,   "bias", i), {n_ff}, 0);
                            layer.ffn_down_b = create_tensor(tn(LLM_TENSOR_FFN_DOWN, "bias", i), {n_embd}, 0);
                        } else if (arch == LLM_ARCH_NOMIC_BERT) {
                            layer.ffn_gate = create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd, n_ff}, 0);
                        } else if (arch == LLM_ARCH_MODERNBERT) {
                            layer.ffn_gate = create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd, n_ff}, 0);
                        }
